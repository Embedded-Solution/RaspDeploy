...
# SCIN360 added apBridgeMode and autoReboot parameters for specific usage 20201125
...
apPassphrase=""
apPasswordConfig=""
apBridgeMode="$apBridgeModeDefault"
autoReboot="$autoRebootDefault"
...
for i in ${!options[@]}; do
    ...
    if [[ "$option" == --ap-bridge-mode=* ]]; then
	apBridgeModeTmp="$(echo $option | awk -F '=' '{print $2}')"
        if [[ ${apBridgeModeTmp} =~ ^(On|on|ON|oN) ]];then
                apBridgeMode="on"
        else
                apBridgeMode="off"
        fi
...
'
	...
	if [ "$apBridgeMode" = false ]; then
echo '
--ap-bridge-mode       Optional field for installation: define bridged network for Access Point(AP) and Client.
                       Default value is: '${apBridgeModeDefault}'
                       on: bridge is active, off: two separate network
'
	fi
...
if [ "$cleanup" = false -a "$install" = false -a "$installUpgrade" = false ]; then
    ...
    --force-reboot          Reboot after installation.
    ...
    --ap-bridge-mode            Optional field for installation: Define brigde between  Access Point(AP) and Wifi Client.
                            Default value is: '$apBridgeModeDefault'
                            If 'on' AP and Client are linked, if 'off' AP and Client are both independant.
